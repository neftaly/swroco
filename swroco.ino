...
void updateScreen() {
  ...
  // Pressure
  // if (pressValue < 0) sprintf(buffer, "Press:   *ERROR*");
  if (false) return;
  else sprintf(buffer, "Press:%6d PSI", (int)pressValue);
  oled.println(buffer);
...
void updatePressure() {
  float voltage = (float)analogRead(PRESS_PIN) * VREF / 1023.0;
